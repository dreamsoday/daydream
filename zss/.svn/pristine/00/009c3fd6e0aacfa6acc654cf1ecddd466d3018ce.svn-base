...
<div class="bodywrapper">
    ...
    <div class="centercontent tables" id="replace">
<?php if($role_id=='1') {?>
...
<?php } ?>
<a href="<?=Yii::$app->urlManager->createUrl(['count/excelout1'])?>&role_id=<?php echo $role_id?>&shop_name=<?php echo $shop_name?>&begintime=<?php echo $begintime?>&endtime=<?php echo $endtime?>">导出</a>
</div><!--contentwrapper-->
</div><!-- centercontent -->
</div><!--bodywrapper-->
</body>
...
jQuery(document).on('click','#searchdate',function()
{
    ...
    var endtime = jQuery('.end').attr('value')
        jQuery.ajax({
                    type: "GET",
                    url: "<?= Yii::$app->urlManager->createUrl(['count/search'])?>",
